...
library(knitr)
opts_chunk$set(fig.path = "images/pitstop-")
```
# Pit Stop Analysis
...
```{r pitStopBar,message=FALSE,fig.cap='Accumulated pit stop times during a single race',fig.width=7,warning=FALSE}
library("ggplot2")
...
```{r pitstopBarDistinct, fig.cap='Bar chart showing pit stop times for each pit event in the Australia 2012, Grand Prix'}
...
```{r pitstopBarDiff, fig.cap='Bar chart showing the difference between consecutive, increasingly ordered, pit stop times for each pit event in the Australia 2012, Grand Prix',warning=FALSE}
library(plyr)
...
```{r pitstopStackedbar,fig.cap='Cumulative pit stop times as a combination of distinct individual pit stop times', fig.width=7}
...
```{r dodgeydodge, fig.cap='A far from ideal dodged bar chart...'}
...
```{r pitstopDodgedbar,fig.cap='Dodged bar chart with empty bars, with drivers ordered by summed pit stop time', fig.width=7}
...
```{r pitstopsByLap,fig.cap='Pit stops by lap', fig.width=7}
...
```{r echo=FALSE}
#Preview the contents of the pitStops table
kable(tail(head(lap_pits,95),5),row.names=F,format='markdown')
```
