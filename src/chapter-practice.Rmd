...
library(knitr)
opts_chunk$set(fig.path = "images/practice-")
```
...
```{r rawTimeVrank, message=FALSE,warning=FALSE, fig.cap='Raw, unordered scatterplot of laptime against rank position'}
...
```{r rawlapcountVdriver, fig.cap='Raw, unordered bar chart showing lap count against driver'}
...
```{r labeledLapVrank, fig.cap='Labelled chart showing laptimes against ranked position'}
dd$pos=as.integer(dd$pos)
...
```{r annotatedLapVrank, fig.cap='Annotated chart showing laptime against ranked position, with horizontal "separation bars"" added manually' }
...
```{r laptimepercent, fig.cap='Laptime shown as a percentage of the fastest lap time, with 107% line added'}
...
```{r lapdeltas, fig.cap='Laptime deltas between consecutively ranked cars'}
...
```{r lapdeltasColoured, fig.cap='Lap delta bar chart with coloured bar edges'}
...
```{r sortedLapcountbarChart, fig.cap='Lap count bar chart ordered by lap count'}
dd$laps = as.integer(dd$laps)
...
```{r horizLapcountbarChart, fig.cap='Horizontal bar chart, with the original x/y axis labels flipped too'}
...
```{r styledLapCount, fig.cap='Styled horizontal bar chart with top 5 ranked drivers highlighted'}
...
```{r lapcountByTeam, fig.cap='Dodged bar chart showing lap count for drivers grouped by team'}
...
```{r totalLapCountByTeam, fig.cap='Stacked bar chart showing total lap count by team'}
g = ggplot(dd)
...
```{r relativeLaptimeSeparation, fig.cap='Chart showing separation of lap times'}
...
```{r relativeLaptimeSeparationFlipped, fig.cap='Flipped and dodged chart showing separation of lap times'}
...
```{r lapSeparationEngineHighlight, fig.cap='Highlighting engine manufacturer using the colour dimension'}
#Basic dot plot
g=ggplot(dd) + geom_point(aes(x=time,y="Time"),shape=1)
#Split the drivers into two groups - odd position number and even position number
#Use each group as a separate y-axis categorical value
...
```{r echo=FALSE}
kable(dbGetQuery(f1, ("SELECT * FROM p1Sectors LIMIT 5")), format='markdown')
```
...
```
```{r echo=FALSE}
kable(head(ult), format='markdown')
```
...
```
```{r echo=FALSE}
kable(head(subset(gp_2012_aus_p1_results, select=-c(year,race)),
      row.names = FALSE, format='markdown' ))
```

```{r rawUltimate, fig.cap='Raw scatterplot showing ultimate versus actual laptime for each driver'}
library(ggplot2)
...
```{r rawultimatetext, fig.cap='Raw text plot showing ultimate vs best laptime for each driver by name'}
...
```{r rawultimatetextdot, fig.cap='Ultimate vs actual laptime with point and text label layers'}
...
```{r rawultimateabline, fig.cap='Addition of line showing equivalent ultimate and actual laptimes'}
...
```{r nooverlaptextdot,fig.cap='Using the ggrepel library to avoid text label overlaps'}
...
```{r ultimateVactualLabelstyle, fig.cap='Highlighting drivers more than 0.3s away from their ultimate laptime'}
g = ggplot(gp_2012_aus_p1_results, aes(x=time, y=ultimate))
...
* the best overall session laptime, `min(r$time)`: `r min(r$time)`;
* the best overall session laptime in a team, `min(r[r$team=='Williams-Renault',]$time)`: `r min(r[r$team=='Williams-Renault',]$time)`;
* the best ultimate lap in a team: `min(r[r$team=='Williams-Renault',]$ultimate)`: `r min(r[r$team=='Williams-Renault',]$ultimate)`;
