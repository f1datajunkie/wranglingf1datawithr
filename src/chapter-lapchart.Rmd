...
```
```{r simpleLapchart,fig.cap='Simple lapchart',fig.width=7, fig.height=6,echo=FALSE}
g
```
...
```{r lapchartZoom,fig.cap='Zooming in on the simple lapchart',fig.width=7, fig.height=6, warning =FALSE}
...
```{r lapChartLap1fixAndLabel,warning=FALSE}
...
g=g+scale_y_reverse()
```
```{r lapchartFlipped,fig.cap='Lapchart flipped to show winning driver at the top right of the chart',fig.width=7, fig.height=6,echo=FALSE}
g
```
...
```{r lapchartUnfinishedAnnotation,fig.cap="Annotating a lapchart to show drivers that didn't finish the race",fig.width=7, fig.height=6,warning=FALSE}
...
g=g+geom_text(data=subset(lapPos.status,status!='Finished'),
            ...
            col='red', alpha=0.5)
```
```{r lapchartWithdrawLabel,fig.cap='Annotating a lapchart to show reasons why drivers withdrew from a race',fig.width=7, fig.height=6, echo=FALSE,warning =FALSE}
g
```
...
```{r lapChartPitEvents,fig.cap="Further annotating the lap chart with pit events",warning=FALSE}
...
```{r lapChartWithGridSketch,fig.cap="Lap chart with initial grid position marked and distinction between grid position and first lap position highlighted by a grey line",warning=FALSE}
...
```{r lapCharterExample, fig.cap="Example of the lapcharter function"}
...
```{r lapPosStatusChart, fig.cap='Summary chart showing the number of laps in each position for each driver in a single race'}
...
```
```{r echo=FALSE}
kable(head(finalPos,5),format='markdown')
```
...
```
```{r lapPosStatusChartOrdered,fig.cap='Lap position summary chart with drivers ordered by final classification. The chart is further annotated with the position the driver was in on their last lap (pink filled circle, size proportional to number of laps completed),  their grid position (empty square) and their position at the end of the first lap (empty circle).',warning=FALSE,echo=FALSE}
g
```
...
```{r raceSummaryLapColourJitter, fig.cap="Hinting at lap number using colour",warning=FALSE}
...
```
```{r echo=FALSE}
kable(head(firstLap,3),format='markdown')
```
...
```{r poschangecountscatter,fig.cap="Count of changes in position against position",warning=FALSE}
...
```{r slopeGraphSketch,fig.cap="Sketch of a simple skop graph comparing grid and final positions",warning=FALSE}
...
```{r slopeGraphExtended,fig.cap="Extended slope graph showing grid, end of lap 1, and final positions", warning=FALSE}
...
```{r sketchPosChangeNames,fig.cap="Text plot sketch showing laps where drivers changed position during a race"}
