...
library(knitr)
opts_chunk$set(fig.path = "images/battlemaps-")
```
...
```{r carahead,fig.cap='For a selected driver, chart the time to the car ahead',warning=FALSE}
library(ggplot2)
...
```{r carlabelahead,fig.cap="For a selected driver, identify which driver is ahead or behind on track, and by how much time, on each lap"}
...
```{r battleplotd1,fig.cap="A first attempt at a battle map, showing cars in positions immediately ahead/behind on track, as well as in race position terms"}
...
```{r battlemapD1,fig.cap="A first attempt at a complete battlemap.",warning=FALSE,message=FALSE}
...
```{r battleCoreWEB, warning=FALSE,message=FALSE}
...
```{r positionbattles,fig.cap='A position battle chart showing the fight for third in the course of a single race', warning=FALSE,message=FALSE}
...
```{r overprintCreditExample, warning=FALSE}
library(grid)
library(gridExtra)
...
```{r trackpositionmap,fig.cap="A track position map showing how cars are separated on track for each lead lap"}
