...
library(knitr)
opts_chunk$set(fig.path = "images/quali-")
...
```{r echo=FALSE}
kable(head(qualiResults[,!names(qualiResults) %in% c('year','race','session')],n=3),format='markdown')
```
...
```{r echo=FALSE}
kable(head(qm,n=3),format='markdown')
```

With the data in shape, let's have a look at it. To start with, we'll order drivers by laptime within each session.

```{r rawSessionTimeName,fig.cap='Text plot showing relative qualifying session times', message=FALSE, warning=FALSE}
library(ggplot2)
...
```{r rawSessionTimeNameLine,fig.cap='Text plot of qualifying session times with driver connector lines',warning=FALSE}
...
```
```{r echo=FALSE}
kable(tail(qm,n=12),row.names=FALSE,format='markdown')
```
...
```{r sessionTableStyled, fig.cap='Session times table, with highlights'}
g = ggplot(qm,aes(x=session,y=laptime)) 
g = g+ geom_line(aes(group=driverName),colour='darkgrey')
...
```{r qualirankBySession,fig.cap='Custom line chart showing rank by qualifying session', message=FALSE}
