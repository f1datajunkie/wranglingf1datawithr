...
library(knitr)
opts_chunk$set(fig.path = "images/qualiUtil-")
...
```{r qsession_utilisation, warning=F}
...
```{r echo=FALSE}
kable(head(f12015test,3),row.names=F,format="markdown")
```
...
```{r qsessionutulisation, warning=FALSE}
...
```{r session_facetscatter_purplegreen, fig.cap=""}
...
```{r qualipatchedSessionUtil,fig.cap="Qualifying session utilisation chart, coloured relative to each part of qualifying using an online algorithm"}
...
```{r cutofftimeEvolution,fig.cap="Charting the evolution of the cutoff time in a qualifying session"}
...
```{r cutofftimeFinal,fig.cap="Annotating the cutoff time evolution chart with the final cutoff time"}
...
```{r missedcutoff,fig.cap="Highlighting drivers' best times that missed the cut"}
