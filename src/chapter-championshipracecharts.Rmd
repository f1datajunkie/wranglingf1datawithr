...
library(knitr)
opts_chunk$set(fig.path = "images/championshipRace-")
```
...
```
```{r echo=FALSE}
kable(head(df,n=3),format='markdown')
```
...
```{r pointsRace, message=FALSE,fig.cap="Cumulative points scored at the end of each round in the 2014 Drivers' Championship"}
...
```
```{r echo=FALSE}
kable(head(df[df$driverId=='rosberg',c('round','points','racepoints')],n=3),format='markdown')
```

We can now use the *points scored per race* information to annotate the chart:

```{r champRaceStandingsPoint,fig.cap="Annotating the championship standings chart with points won per race"}
...
```{r champRaceStandingsPointCol,fig.cap="Highlighting the points using colour"}
